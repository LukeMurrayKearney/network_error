...
impl NetworkStructure {
    ...
    pub fn new_multinomial(n: usize, partitions: &Vec<usize>, period: &str) -> (NetworkStructure, (usize,usize,Vec<Vec<f64>>)) {
        ...
        for (part_i, &part_i_end) in partitions.iter().enumerate() {
            ...
            for (part_j, &part_j_end) in partitions.iter().enumerate().take(part_i+1) {
                ...
                if part_i == part_j {
                    (tmp_edges, missing_links, stubs_remaining) = connect_stubs_diagonal(&nodes_i, &mut rng);
                    total_unconnected_stubs += missing_links.0;
                }
                else {
                    (tmp_edges, missing_links, stubs_remaining) = connect_stubs(&nodes_i, &nodes_j, &mut rng);
